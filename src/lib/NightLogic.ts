import { Player, ChatMessage, callAI } from "./GameLogic";
import { werewolfPrompt } from "./Prompts";

export async function werewolfSpeak(player: Player, wolfChat: ChatMessage[], players: Player[]): Promise<string> {
  const messages = [
    { role: "system", content: werewolfPrompt(player, players, wolfChat) },
    ...wolfChat.map((m) => ({ role: "assistant", content: `${m.name}: ${m.text}` })),
    { role: "user", content: `You whisper to your partner. What do you say?` },
  ];
  return callAI(messages);
}

export async function startNight(
  players: Player[],
  priorChat: ChatMessage[],
  round: number
): Promise<{ updatedPlayers: Player[]; nightChat: ChatMessage[] }> {
  const playersCopy = players.map((p) => ({ ...p }));
  const wolves = playersCopy.filter((p) => p.alive && p.role === "werewolf");
  const villagers = playersCopy.filter((p) => p.alive && p.role !== "werewolf");

  const wolfChat: ChatMessage[] = [];
  const nightChat: ChatMessage[] = [...priorChat, { id: 0, name: "Narrator", text: `Night ${round} falls. Werewolves whisper.` }];
  let chosen: Player | null = null;

  for (let r = 0; r < 4 && !chosen; r++) {
    for (const wolf of wolves) {
      const reply = await werewolfSpeak(wolf, wolfChat, playersCopy);
      wolfChat.push({ id: wolf.id, name: wolf.name, text: reply });
      nightChat.push({ id: wolf.id, name: `${wolf.name} (wolf whisper)`, text: reply });

      for (const v of villagers) {
        const mentions = wolfChat.filter((m) => m.text.toLowerCase().includes(v.name.toLowerCase()));
        const uniqueMentioners = new Set(mentions.map((m) => m.name));
        if (uniqueMentioners.size >= wolves.length) {
          chosen = v; break;
        }
      }
      if (chosen) break;
    }
  }

  if (!chosen && villagers.length > 0) {
    chosen = villagers[Math.floor(Math.random() * villagers.length)];
    nightChat.push({ id: 0, name: "Narrator", text: `Werewolves pick ${chosen.name}.` });
  }

  if (chosen) {
    playersCopy.forEach((p) => { if (p.name === chosen!.name) p.alive = false; });
    nightChat.push({ id: 0, name: "Narrator", text: `${chosen.name} was killed.` });
  }

  return { updatedPlayers: playersCopy, nightChat };
}
